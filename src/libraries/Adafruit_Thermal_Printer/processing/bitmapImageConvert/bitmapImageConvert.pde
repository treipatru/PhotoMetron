...
void processImage(File image) {
  ...
  int         pixelNum, byteNum, bytesOnLine = 99,
              x, y, b, z, rowBytes, totalBytes, lastBit, sum;
  ...
  shortname = filename.substring(z+1,x);
  ...
  output.println("#ifndef " + shortname + "_h_");
  output.println("#define " + shortname + "_h_");
  output.println();
  output.println("#define " + shortname + "_width  " + img.width);
  output.println("#define " + shortname + "_height " + img.height);
  output.println();
  output.print("static const uint8_t PROGMEM " + shortname + "_data[] = {");
  ...
  output.println("};");
  output.println();
  output.println("#endif // _" + shortname + "_h_");
  output.flush();
  output.close();
  println("Done!");
  exit();
